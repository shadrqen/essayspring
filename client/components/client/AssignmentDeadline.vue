...
<!--  Uses the deadline prop to determine whether to show this span element or not-->
    <span :style="{ color: color }" v-if="deadline">
      <span v-if="deadline.days !== 0">
        <span v-if="Math.sign(deadline.days) === -1" style="color: red">
      ...
      <span v-if="deadline.hours !== 0">
        <span v-if="Math.sign(deadline.hours) === -1" style="color: #ff0000">
          <span v-if="Math.abs(deadline.days + 1) === 0">
            Past due time by
          </span>
        ...
        <span v-else>
          <span v-if="deadline.hours === 1" :style="deadline.days === 0 ? 'color: #D01C1C' : ''">
           {{ String(deadline.hours) }} hour left
          </span>
          <span v-else :style="deadline.days === 0 && deadline.hours < 4 ? 'color: #D01C1C' : ''">
