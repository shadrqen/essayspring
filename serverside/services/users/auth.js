...
class AuthService {
    ...
    */
    static generateAccessToken = (email, expiry_minutes, userType) => jwt.sign({
        sub: email,
        user: userType
    }, authSecretKey, {expiresIn: expiry_minutes});
                    ...
                    if (user.user.password || ['Facebook', 'Google'].includes(user.loginVia)) {
                        ...
                        * The user has to continue using email for authentication */
                        ...
                        if (match) {
                            ...
                            let accessToken = AuthService.generateAccessToken(email, '15min', 'Client')
                            let refreshToken = AuthService.generateRefreshToken(email, 'Client')
                            return {
                                message: 'success',
                                userType: user.type,
